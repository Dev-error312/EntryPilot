...
export class ConfidenceCalculator {
  ...
  calculateFieldConfidence(
    ...
    const field = CHINA_VISA_FIELDS.find(f => f.id === fieldName);
    if (!field) return 50; // Unknown field
   ...
   */
  private getFormatRegex(format: string): RegExp | null {
  ...
  getCompletenessScore(applicant: Record<string, any>): number {
    const requiredFields = CHINA_VISA_FIELDS.filter(f => f.required);

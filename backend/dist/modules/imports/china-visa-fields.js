...
exports.CHINA_VISA_FIELDS = void 0;
...
exports.findFieldByOCRText = findFieldByOCRText;
// All fields from China Visa Form V.2013
exports.CHINA_VISA_FIELDS = [
...
function getFieldsBySection(section) {
    return exports.CHINA_VISA_FIELDS.filter(f => f.section === section);
}
// Get all unique sections
function getAllSections() {
    return [...new Set(exports.CHINA_VISA_FIELDS.map(f => f.section))];
}
// Find field by Excel column name
function findFieldByExcelColumn(columnName) {
    const normalized = columnName.toLowerCase().replace(/[\s\-_]+/g, '_').trim();
    return exports.CHINA_VISA_FIELDS.find(f => f.excelAliases?.some(alias => alias.toLowerCase().replace(/[\s\-_]+/g, '_') === normalized) || f.id === normalized);
}
// Find field by OCR text
function findFieldByOCRText(text) {
    for (const field of exports.CHINA_VISA_FIELDS) {
...
//# sourceMappingURL=china-visa-fields.js.map
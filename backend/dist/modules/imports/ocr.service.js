...
class OCRService {
    ...
    async preprocessImage(imagePath, options) {
        const { deskew = true, contrast = 1.5, removeNoise = true, handwritingOptimized = false } = options || {};
     ...
     */
    async recognizeWithConfidence(imagePath, optimize) {
        ...
        if (preprocessing) {
            ...
            imageBuffer = await promises_1.default.readFile(imagePath);
        }
            ...
            if (data.lines) {
                for (const line of data.lines) {
                    const lineConfidence = line.words?.length
                        ? Math.round(line.words.reduce((sum, w) => sum + w.confidence, 0) /
                            line.words.length)
                        : 0;
                    result.lines.push({
                        text: line.text,
     ...
     */
    async extractHandwritingWithConfidence(imagePath) {
        ...
        const tempPath = path_1.default.join('/tmp', `temp_${Date.now()}_handwriting.png`);
        await promises_1.default.writeFile(tempPath, preprocessed);
        try {
            ...
            const avgConfidence = wordConfidences.length > 0
                ? Math.round(wordConfidences.reduce((a, b) => a + b, 0) / wordConfidences.length)
                : 0;
            // Calculate variance (handwriting = higher variance)
            const variance = wordConfidences.length > 0
                ? Math.sqrt(wordConfidences.reduce((sum, conf) => sum + Math.pow(conf - avgConfidence, 2), 0) / wordConfidences.length)
                : 0;
     ...
     */
    async adjustContrast(imageBuffer, factor) {

...
exports.transformToApplicant = transformToApplicant;
...
function parseExcelData(rows) {
    ...
    headers.forEach((header, index) => {
        const field = (0, china_visa_fields_1.findFieldByExcelColumn)(header);
        if (field) {
...
function parseExcelRow(row, headerMapping, rowNumber) {
    const applicant = {
        data: {},
        confidence: 100,
        fieldConfidence: {},
    ...
    china_visa_fields_1.CHINA_VISA_FIELDS.forEach(field => {
...
function parsePDFText(text) {
    ...
    for (const line of lines) {
        const match = (0, china_visa_fields_1.findFieldByOCRText)(line);
        if (match) {
...
function normalizeSelectValue(field, value) {
    ...
    if (exactMatch)
        return exactMatch;
    // Partial match
    const partialMatch = field.options.find(o => o.toLowerCase().includes(normalized) ||
...
function extractCommonPatterns(text, applicant) {
    // Extract email
...
function transformToApplicant(data, groupId) {
    return {
...
//# sourceMappingURL=china-visa-parser.js.map
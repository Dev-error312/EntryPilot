...
exports.VISA_APPLICATION_FIELDS = void 0;
exports.findFieldByExcelColumn = findFieldByExcelColumn;
exports.findFieldByPDFField = findFieldByPDFField;
...
exports.VISA_APPLICATION_FIELDS = [
    // DOCUMENTS
    {
        id: 'visaPhoto',
        name: 'visaPhotoId',
...
function findFieldByExcelColumn(headerText) {
    const normalized = headerText.toLowerCase().trim();
    for (const field of exports.VISA_APPLICATION_FIELDS) {
        ...
        for (const column of field.excelColumns) {
            if (column.toLowerCase().includes(normalized) || normalized.includes(column.toLowerCase())) {
                return field;
            }
        }
    }
...
function findFieldByPDFField(pdfFieldName) {
...
function getFieldsBySection(section) {
    return exports.VISA_APPLICATION_FIELDS.filter(f => f.section === section);
}
function getRequiredFields() {
    return exports.VISA_APPLICATION_FIELDS.filter(f => f.required);
}
function getAllSections() {
    const sections = new Set();
    exports.VISA_APPLICATION_FIELDS.forEach(f => sections.add(f.section));
...
//# sourceMappingURL=visa-application-fields.js.map
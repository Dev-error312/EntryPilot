...
export default function VisaApplicationForm({
  ...
  const [formData, setFormData] = useState<VisaFormData>(() => ({
    ...
    otherNationalityIdNumber: undefined,
    otherNationalityPassportNumber: undefined,
    otherNationalityNotProvidedReason: undefined,
    hasPermanentResidence: false,
    permanentResidenceCountries: undefined,
    hasFormerNationality: false,
    formerNationality: undefined,
    currentOccupation: '',
    workExperienceStartDate: undefined,
    workExperienceEndDate: undefined,
    ...
    previousChineseVisaFiles: undefined,
    hasValidVisas: false,
    validVisaCountries: undefined,
    countriesVisitedLast12Months: undefined,
    groupId: '',
    ...(initialData || {} as Partial<VisaFormData>),
  }));

  const containerClass = inline ? 'bg-white rounded-xl shadow-sm border border-gray-200 p-6' : 'min-h-screen bg-gray-50 py-8 px-4';
  ...
  const handleInputChange = (field: keyof VisaFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleFileChange = (field: keyof VisaFormData, files: FileList | null) => {
    if (!files) return;
  ...
  const submitForm = async () => {
    try {
      ...
      // Create VisaFormData for multipart upload
      ...
      if (formData.visaPhoto) {
        uploadFormData.append('visaPhoto', formData.visaPhoto as any);
      }
      if (formData.passportPage1) {
        (formData.passportPage1 as any[]).forEach((file) => {
          uploadFormData.append(`passportPage1`, file);
        });
      }
      if (formData.passportPage2) {
        (formData.passportPage2 as any[]).forEach((file) => {
          uploadFormData.append(`passportPage2`, file);
        });
      }
      if (formData.passportBackPage) {
        (formData.passportBackPage as any[]).forEach((file) => {
          uploadFormData.append(`passportBackPage`, file);
        });
      }
      if ((formData as any).previousChineseVisaFiles) {
        (formData as any).previousChineseVisaFiles.forEach((file: File) => {
  ...
  return (
    <div className={containerClass}>
      <div className={inline ? 'max-w-full' : 'max-w-3xl mx-auto'}>
        {/* Header */}
        <div className="mb-8">
...
function DocumentSection({
  ...
  handleFileChange: (field: keyof VisaFormData, files: FileList | null) => void;
  fileInputRefs: any;
}) {
  return (
...
function PersonalInfoSection({
  ...
  formData: VisaFormData;
  handleInputChange: (field: keyof VisaFormData, value: any) => void;
}) {
  return (
